...
for a in range(0, 50000, 1000):
  ...
  add_record = ("INSERT INTO dicionary_test1 (word, w2vec, idf) VALUES (%s, %s, %s, %s)") 
  
  n=3
  splitted_array=[finals[i:i + n] for i in range(0, len(finals), n)]

  for data in splitted_array:
  ...
  for index, final in enumerate(finals):
    ...
    cursor.executemany(add_record, data)
    cnx.commit() 
    data.append((question['id'], corpus1[index], corpus2[index], question['tags']))
