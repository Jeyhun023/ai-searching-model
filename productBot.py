...
# /var/local/python/
f = open("/home/abysshub_bots/devto.txt", "r")
counter = f.read()
f.close()
...
try:
    data = requests.get(f'https://dev.to/search/feed_content?per_page=15&page={counter}&tag=javascript&sort_by=hotness_score&sort_direction=desc&tag_names%5B%5D=javascript&approved=&class_name=Article').json()
    print('getting list')
...
f = open("/home/abysshub_bots/devto.txt", "w")  
